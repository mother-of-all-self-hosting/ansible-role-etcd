...
#
# Tags look like `v<etcd version>-<release>`:
#
# - if defaults/main.yml points at an etcd version that has never been
#   released, the release counter restarts at 0 (`v3.6.4-0`)
# - otherwise the counter is incremented (`v3.6.4-1`), but only if something
...
if [ -z "$version" ]; then
	echo >&2 "Could not determine the etcd version from $defaults_path"
	exit 1
fi
